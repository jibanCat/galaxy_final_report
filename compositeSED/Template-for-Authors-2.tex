...
\begin{algorithm}[H]
    ...
    \tcp{define 22 rest-frame filters with equal widths between 1\,226 \AA $ < \lambda < $ 49\,580 \AA}
    $ Bin(\mathbf{\lambda}) \leftarrow \mathrm{MAP} \mathbf{\lambda} : \mathbf{\lambda}[\log_{10}{1226}: \frac{1}{22} (\log_{10}{49580} - \log_{10}{1226}) i] $
    
    % fit with EAZY: determine redshift
    \tcp{look through each galaxy multi-wavelength vector, d, and fit with EAZY}
    \For{$\mathbf{f} \leftarrow \mathbf{F}$}{
        \tcp{fit with EAZY}
